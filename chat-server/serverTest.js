...
async function storePrivateMessage(msg) {
  const key = [msg.senderId, msg.target].sort().join("_");
...
<<<<<<< HEAD
  ...
  socket.on("private message", (msg) => {
    ...
    const receiver = users.get(msg.receiverUsername);
    ...
    // 存储消息
    const storageKey = [sender.username, receiver.username].sort().join("_");
    if (!privateMessages.has(storageKey)) {
      privateMessages.set(storageKey, []);
    }
    privateMessages.get(storageKey).push(messageObj);
  ...
  socket.on("load_private_history", async ({ targetUsername }) => {
    ...
    const storageKey = [currentUser, targetUsername].sort().join("_");
